...
public class MemberDAO {
	...
	public List<MemberVO> saerchMember2(MemberVO memberVO) {
		
		sqlMapper =getInstance();
		SqlSession session = sqlMapper.openSession();
		
		List<MemberVO> listmember = session.selectList("mapper.member.saerchMember2", memberVO);
		session.commit();
		return listmember;
	...
	public void modMember(MemberVO memberVO) {
		...
		session.update("mapper.member.modMember", memberVO);
		session.commit();
